...
Single-file CRUD information thermodynamics demo with:
  ...
  - Optional underdamped Langevin robustness check for reviewer response
...
VERSION = "CRUD-FIG-FINAL-v1.3-underdamped"
...
# Optional underdamped robustness check for reviewer response.
# This is a supplementary numerical check, not the primary model.
ENABLE_UNDERDAMPED_CHECK = True
UD_MASS = 1.0
UD_GAMMA = 1.0
UD_DT = 0.0005
UD_NPART = 6000
UD_N_LAMBDA_STEPS = 2000
UD_RELAX_STEPS = 8
UD_DKL_BINS = DKL_BINS
...
if ENABLE_UNDERDAMPED_CHECK:
    print(f"Underdamped robustness check: mass={UD_MASS}, gamma={UD_GAMMA}, dt={UD_DT}, N={UD_NPART}")
print("=" * 72)
...
class GPUPowerLogger:
    ...
    def summary(self, idle_power_W=None):
        ...
        if idle_power_W is not None and np.isfinite(float(idle_power_W)):
            idle_power_W = float(idle_power_W)
            p_net = np.maximum(p - idle_power_W, 0.0)
            net_energy_J = float(np.trapezoid(p_net, t))
            net_mean_power_W = float(np.mean(p_net))
        else:
            idle_power_W = None
            net_energy_J = None
            net_mean_power_W = None

        return {
            "gpu_logger_available": True,
            "gpu_samples": int(len(valid)),
            "gpu_energy_J": energy_J,
            "gpu_raw_energy_J": energy_J,
            "gpu_idle_power_W": idle_power_W,
            "gpu_idle_subtracted_energy_J": net_energy_J,
            "gpu_net_mean_power_W": net_mean_power_W,
...
@jit
def step_underdamped_single(x, v, key, b, c, T, mass, gamma_ud, dt_ud):
    """
    One Euler-Maruyama step for underdamped Langevin dynamics:

        dx = v dt
        m dv = [F(x;lambda) - gamma v] dt + sqrt(2 gamma kT) dW

    The protocol controls only the potential parameters (b, c). The mass is
    fixed, so the kinetic partition factor cancels between endpoints.
    """
    noise = random.normal(key)
    f = force(x, b, c)
    v_new = v + ((f - gamma_ud * v) / mass) * dt_ud \
        + (jnp.sqrt(2.0 * gamma_ud * KB * T) / mass) * jnp.sqrt(dt_ud) * noise
    x_new = x + v_new * dt_ud
    return x_new, v_new

@jit
def step_underdamped_batch(x_batch, v_batch, keys, b, c, T, mass, gamma_ud, dt_ud):
    return vmap(
        lambda x, v, k: step_underdamped_single(x, v, k, b, c, T, mass, gamma_ud, dt_ud)
...
def simulate_protocol_underdamped(protocol_fn, x0, n_lambda_steps, relax_steps, T, key,
                                  mass=UD_MASS, gamma_ud=UD_GAMMA, dt_ud=UD_DT):
    """
    Underdamped counterpart of simulate_protocol.

    Work is still accumulated from changes in the externally controlled
    potential V(x; b, c). The kinetic term m v^2/2 has no protocol parameter
    dependence because mass is held fixed.
    """
    bs = np.zeros(n_lambda_steps + 1, dtype=np.float64)
    cs = np.zeros(n_lambda_steps + 1, dtype=np.float64)
    for i in range(n_lambda_steps + 1):
    ...
    x = jnp.array(x0, dtype=jnp.float32)
    sigma_v = np.sqrt(KB * T / float(mass))
    v = random.normal(kv, shape=x.shape) * sigma_v
    W = jnp.zeros(x.shape[0], dtype=jnp.float32)
    ...
    for i in range(n_lambda_steps):
        ...
        for _ in range(int(relax_steps)):
            key, sub = random.split(key)
            keys = random.split(sub, x.shape[0])
            x, v = step_underdamped_batch(
                x, v, keys, b_new, c_new, T,
                float(mass), float(gamma_ud), float(dt_ud)
            )
...
def run_crud_operation_underdamped(op_name, *,
                                  b_hi, b_lo, c_mag, unbias_frac, end_mode,
                                  n_lambda_steps, relax_steps, n_particles, T, key,
                                  dkl_bins=UD_DKL_BINS, mass=UD_MASS,
                                  gamma_ud=UD_GAMMA, dt_ud=UD_DT):
    """
    Underdamped robustness-check runner for Create/Update/Delete.

    The memory classification is evaluated from the position marginal x, using
    the same macro/intra KL accounting as the overdamped model. The velocity
    degree of freedom is included dynamically and initialized thermally.
    """
    proto, init_region, target = make_protocol(op_name, b_hi, b_lo, c_mag, unbias_frac, end_mode)
    b0, c0 = proto(0.0)
    b1, c1 = proto(1.0)
    ...
    dkl_init = compute_dkl_full_macro_intra(x0, float(b0), float(c0), T, n_bins=dkl_bins)
    x1, v1, W_samples = simulate_protocol_underdamped(
        proto, x0, n_lambda_steps, relax_steps, T, k_run,
        mass=mass, gamma_ud=gamma_ud, dt_ud=dt_ud,
    )
    dkl_final = compute_dkl_full_macro_intra(x1, float(b1), float(c1), T, n_bins=dkl_bins)
    ...
    # The kinetic partition factor cancels because mass and temperature are
    # unchanged between endpoints; only the potential equilibrium term remains.
    deltaF_eq = free_energy_eq(float(b1), float(c1), T, region=None) - free_energy_eq(float(b0), float(c0), T, region=None)
    ...
    note = {
        "Create": "underdamped write-like / known-to-known",
        "Update": "underdamped overwrite-like / unknown-to-target",
        "Delete": "underdamped landauer-like / unknown-to-blank0",
    }[op_name]
...
def run_underdamped_robustness(*, core_results, c_write, T, key):
    """
    Supplementary robustness check for Reviewer #1(h).

    It tests whether including inertia changes the qualitative CRUD
    classification. It is not used to replace the primary overdamped model.
    """
    rows = []
    ...
    print()
    print("=" * 72)
    print("UNDERDAMPED ROBUSTNESS CHECK")
    print("=" * 72)
    print(f"mass={UD_MASS}, gamma={UD_GAMMA}, dt={UD_DT}, n_lambda={UD_N_LAMBDA_STEPS}, relax={UD_RELAX_STEPS}, N={UD_NPART}")
    print("This check includes inertia dynamically but uses the same position-marginal KL classification.")

    for op in ["Create", "Update", "Delete"]:
        key, sub = random.split(key)
        c_use = c_write if op in ("Create", "Update") else C_MAG_DEFAULT
        r = run_crud_operation_underdamped(
            op,
            b_hi=B_HI,
            b_lo=B_LO,
            c_mag=c_use,
            unbias_frac=UNBIAS_FRAC,
            end_mode=END_MODE,
            n_lambda_steps=UD_N_LAMBDA_STEPS,
            relax_steps=UD_RELAX_STEPS,
            n_particles=UD_NPART,
            T=T,
            key=sub,
            dkl_bins=UD_DKL_BINS,
            mass=UD_MASS,
            gamma_ud=UD_GAMMA,
            dt_ud=UD_DT,
        )
        ...
        if od is None:
            ...
            od_text = f" | overdamped DeltaF_total={od['deltaF_total']:+.4f}, W_diss={od['W_diss']:+.4f}"

        print(
            f"  {op:8} <W>={r['meanW']:+.4f}  DeltaF_total={r['deltaF_total']:+.4f}  "
            f"W_diss={r['W_diss']:+.4f}  eps={r['eps']:.4f}" + od_text
        )

    print("Underdamped check complete: compare classification by DeltaF_total, not equality of W_diss.")
    return rows
...
def export_underdamped_csv(underdamped_rows, prefix=PFX):
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    fn = f"{prefix}_underdamped_robustness_{ts}.csv"
    ...
    with open(fn, "w", newline="") as f:
        ...
        w.writerow([
            ...
            "deltaF_eq", "deltaF_info_full", "deltaF_macro", "deltaF_intra",
            "deltaF_total", "W_diss",
            "eps", "eps_se", "pR_init", "pR_final",
            "DKL_init_full", "DKL_final_full",
            "DKL_init_macro", "DKL_final_macro",
            "DKL_init_intra", "DKL_final_intra",
            "recon_err_init", "recon_err_final",
            "mass", "gamma_ud", "dt_ud",
            "relax_steps", "n_lambda_steps", "n_particles",
        ])
        for r in underdamped_rows:
            w.writerow([
                r["op"], r["dynamics"], r["note"],
                r["meanW"], r["stdW"], r["semW"],
                r["deltaF_eq"], r["deltaF_info_full"], r["deltaF_macro"], r["deltaF_intra"],
                r["deltaF_total"], r["W_diss"],
                r["eps"], r["eps_se"], r["pR_init"], r["pR_final"],
                r["DKL_init_full"], r["DKL_final_full"],
                r["DKL_init_macro"], r["DKL_final_macro"],
                r["DKL_init_intra"], r["DKL_final_intra"],
                r["recon_err_init"], r["recon_err_final"],
                r["mass"], r["gamma_ud"], r["dt_ud"],
                r["relax_steps"], r["n_lambda_steps"], r["n_particles"],
            ])
...
def plot_underdamped_robustness(core_results, underdamped_rows, prefix=PFX):
    if not underdamped_rows:
        return

    core_map = {r["op"]: r for r in core_results}
    ud_map = {r["op"]: r for r in underdamped_rows}
    ops = [op for op in ["Create", "Update", "Delete"] if op in core_map and op in ud_map]
    x = np.arange(len(ops))
    width = 0.35
    ...
    ax = axes[0]
    ax.bar(x - width / 2, [core_map[o]["deltaF_total"] for o in ops], width, label="overdamped")
    ax.bar(x + width / 2, [ud_map[o]["deltaF_total"] for o in ops], width, label="underdamped")
    ax.axhline(KB * T0 * np.log(2.0), linestyle="--", linewidth=1.0, label="kT ln2")
    ax.axhline(0.0, linewidth=0.7)
    ax.set_xticks(x)
    ax.set_xticklabels(ops)
    ax.set_ylabel("DeltaF_total")
    ax.set_title("State-function classification")
    ax.grid(True, alpha=0.25, axis="y")
    ax.legend(fontsize=8)

    ax = axes[1]
    ax.bar(x - width / 2, [core_map[o]["W_diss"] for o in ops], width, label="overdamped")
    ax.bar(x + width / 2, [ud_map[o]["W_diss"] for o in ops], width, label="underdamped")
    ax.axhline(0.0, linewidth=0.7)
    ax.set_xticks(x)
    ax.set_xticklabels(ops)
    ax.set_ylabel("W_diss")
    ax.set_title("Protocol-dependent dissipation")
    ax.grid(True, alpha=0.25, axis="y")
    ax.legend(fontsize=8)

    plt.tight_layout()
    plt.savefig(f"{prefix}_underdamped_robustness.png", dpi=150, bbox_inches="tight")
    plt.savefig(f"{prefix}_underdamped_robustness.pdf", dpi=300, bbox_inches="tight")
    plt.close(fig)

    print(f"Saved: {prefix}_underdamped_robustness.png/pdf")
...
def main():
    ...
    # Underdamped robustness check for Reviewer #1(h)
    underdamped_rows = []
    if ENABLE_UNDERDAMPED_CHECK:
        key, sub = random.split(key)
        underdamped_rows = run_underdamped_robustness(
    ...
    if ENABLE_UNDERDAMPED_CHECK:
        export_underdamped_csv(underdamped_rows, prefix=PFX)
    ...
    if ENABLE_UNDERDAMPED_CHECK:
        plot_underdamped_robustness(core_results, underdamped_rows, prefix=PFX)
    ...
    if gpu_logger is not None:
        ...
        config = {
            ...
            "ENABLE_UNDERDAMPED_CHECK": ENABLE_UNDERDAMPED_CHECK,
            "UD_MASS": UD_MASS,
            "UD_GAMMA": UD_GAMMA,
            "UD_DT": UD_DT,
            "UD_NPART": UD_NPART,
            "UD_N_LAMBDA_STEPS": UD_N_LAMBDA_STEPS,
            "UD_RELAX_STEPS": UD_RELAX_STEPS,
            "UD_DKL_BINS": UD_DKL_BINS,
    ...
    return core_results, read_result, scan_rows, write_calib_map, sensitivity_rows, underdamped_rows, gpu_summary
